...
///
/// The script records its PID (= process-group ID after `process_group(0)`) to a
/// sidecar file, backgrounds a 60-second sleep that inherits stdout, then exits
/// with the expected version string.  Three assertions:
///   (a) bounded return — would hang ~60 s if tempfile transport regressed to pipe;
///   (b) correct result;
///   (c) process group dead after return — would pass with a surviving descendant
///       if `process_group(0)` were dropped (kill(-pid) targets only the leader's
///       group; without the call the sleep escapes).
#[cfg(unix)]
#[test]
fn test_probe_node_descendant_holds_stdout_returns_promptly_and_kills_group() {
    ...
    // Write the script's PID (= PGID after process_group(0)) to a sidecar
    // file so the test can check the group is dead after probe_node returns.
    ...
    // (a) bounded return — must not hang on descendant's pipe hold
    assert!(
    ...
    // (c) process group dead — give the group-kill a moment to propagate
    std::thread::sleep(std::time::Duration::from_millis(200));
    let pgid_str = std::fs::read_to_string(&pgid_file)
        .expect("script must have written its PID to the sidecar file");
    let pgid: i32 = pgid_str
        .trim()
        .parse()
        .expect("sidecar file must contain a numeric PID");
    // kill(0) to -pgid: returns 0 if any process in the group is alive, ESRCH if not.
    let group_alive = unsafe { libc::kill(-pgid, 0) } == 0;
    assert!(
        !group_alive,
        "process group {pgid} must be dead after probe_node — \
         if process_group(0) is missing, kill(-pid) targets only the leader's original group"
    );
}
